...
const getAuthToken = () => localStorage.getItem("authToken"); // Corrected key
...
function Products() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (!token) return;
    try {
      // FIX #2: Removed trailing space from URL
  ...
  const handleDelete = (id) => {
    ...
    // FIX #3: Corrected the API endpoint for deletion
  ...
  const columns = [
    { name: "NO.", selector: (row, i) => i + 1, width: "5%", center: true },
    {
      name: "IMAGE",
      cell: (row) => {
        // FIX #1: Use the full URL from the API directly.
