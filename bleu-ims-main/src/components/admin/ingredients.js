...
const getAuthToken = () => localStorage.getItem("authToken"); // Corrected key to match Products.js
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="action-buttons">
                    <button className="action-button view" title="View" onClick={() => handleView(row)}><FaFolderOpen /></button>
                    <button className="action-button edit" title="Edit" onClick={() => handleEdit(row)}><FaEdit /></button>
                    <button className="action-button delete" title="Delete" onClick={() => handleDelete(row.IngredientID)}><FaArchive /></button>
                </div>
            ),
            ignoreRowClick: true,
            width: "15%",
