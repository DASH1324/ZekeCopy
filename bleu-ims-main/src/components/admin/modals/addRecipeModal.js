...
function AddRecipeModal({ onClose, onSubmit, type, products, initialIngredients, supplies: availableSupplies }) {
    ...
    const handleAddIngredient = () => setIngredients([...ingredients, { name: "", amount: "", measurement: "" }]);
    ...
    const handleAddSupply = () => setRecipeSupplies([...recipeSupplies, { name: "", amount: "", measurement: "" }]);
    const handleSupplyChange = (index, field, value) => { const newSupplies = [...recipeSupplies]; newSupplies[index][field] = value; setRecipeSupplies(newSupplies); };
    const handleRemoveSupply = (index) => setRecipeSupplies(recipeSupplies.filter((_, i) => i !== index));
    const handleFocus = (field) => setErrors((prev) => ({ ...prev, [field]: "" }));
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            const token = getAuthToken();
            if (!token) { alert("Authentication token not found."); handleLogout(); return; }
            const formattedIngredients = ingredients.map(ing => ({ IngredientID: parseInt(ing.name, 10), Amount: parseFloat(ing.amount), Unit: ing.measurement || null }));
            const formattedSupplies = recipeSupplies.map(sup => ({ MaterialID: parseInt(sup.name, 10), Quantity: parseFloat(sup.amount), Unit: sup.measurement || null }));
            const newRecipe = { RecipeName: recipeName, ProductID: parseInt(product, 10), Ingredients: formattedIngredients, Materials: formattedSupplies };
            try {
                const response = await fetch(`${API_BASE_URL}/recipes/recipes/`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(newRecipe) });
                if (!response.ok) { const errorData = await response.json(); const errorMessage = errorData.detail ? JSON.stringify(errorData.detail, null, 2) : "An unknown error occurred."; throw new Error(errorMessage); }
                const result = await response.json();
                if (onSubmit) onSubmit(result);
                onClose();
            } catch (error) { console.error("Error adding recipe:", error.message); alert(`Failed to add recipe. Server response:\n\n${error.message}`); }
        }
    };
                        ...
                        {ingredients.length > 0 && ingredients.map((ingredient, index) => (
                            <div key={index} className="recipe-item">
                                <div className="recipe-item-row">
                                    ...
                                    <div className="recipe-item-field"><label>Units</label><input type="text" placeholder="(Optional)" value={ingredient.measurement} onChange={(e) => handleIngredientChange(index, 'measurement', e.target.value)} /></div>
                        ...
                        {recipeSupplies.length > 0 && recipeSupplies.map((supply, index) => (
                            <div key={index} className="recipe-item">
                                <div className="recipe-item-row">
                                    ...
                                    <div className="recipe-item-field"><label>Units</label><input type="text" placeholder="(Optional)" value={supply.measurement} onChange={(e) => handleSupplyChange(index, 'measurement', e.target.value)} /></div>
