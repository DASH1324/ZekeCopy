...
// Use the same token key as the parent component
const getAuthToken = () => localStorage.getItem("authToken");

function AddIngredientModal({ onClose, onSuccess }) { // Changed prop to onSuccess for consistency
    ...
    const [errors, setErrors] = useState({
        ...
        amount: "",
        measurement: "",
        bestBeforeDate: "", // Corrected key from batchDate
        expirationDate: "",
    });
    ...
    useEffect(() => {
        ...
        if (!token || !username) {
            console.log("No session found in modal. Redirecting to login.");
            handleLogout();
        }
    }, [handleLogout]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            ...
            try {
                ...
                if (onSuccess) onSuccess(); // Call the success handler from the parent
        ...
        <div className="addIngredient-modal-overlay">
            <div className="addIngredient-modal-container">
                <div className="addIngredient-modal-header">
                    ...
                    <span className="addIngredient-close-button" onClick={onClose}>×</span>
                </div>
