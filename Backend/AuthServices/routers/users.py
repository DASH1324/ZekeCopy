...
async def create_admin_user():
    ...
    if not admin_user:
        ...
        try:
            await cursor.execute(
                '''INSERT INTO Users (FullName, Username, UserPassword, Email, UserRole, System, isDisabled) 
                   VALUES (?, ?, ?, ?, ?, ?,?)''',
                ('Super Admin', 'superadmin', hashed_password, 'superadmin@example.com', 'superadmin', 'AUTH', 0)
            )
            await conn.commit()
