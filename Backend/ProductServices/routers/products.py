...
@router.get("/products/details/", response_model=List[ProductDetailWithStatusOut], tags=["Inventory Management System - Products"])
async def get_all_full_product_details(token: str = Depends(oauth2_scheme)):
    ...
    try:
        # Step 1: Fetch all product details including calculated status in a single query
        async with conn.cursor() as cursor:
            # A product is 'Unavailable' if it's linked to a recipe that has at least one ingredient
            # whose status is not 'Available'. Otherwise, it's 'Available'.
            ...
            # Step 2: Fetch all sizes in a separate query for efficiency
    ...
    # Step 3: Create an efficient lookup map for sizes
    sizes_by_product_id = {}
    for size in all_sizes_from_db:
        sizes_by_product_id.setdefault(size.ProductID, []).append(size.SizeName)

    # Step 4: Combine data into the final response model
    final_product_list = []
...
@router.get("/products/{product_id}/details", response_model=ProductDetailWithStatusOut, tags=["Inventory Management System - Products"])
