        ...
        async with conn.cursor() as cursor:
            ...
            for recipe in recipes:
                await cursor.execute("""
                    SELECT ri.RecipeIngredientID, ri.Amount, i.IngredientName, ri.Unit 
                ...
                await cursor.execute("""
                    SELECT rm.RecipeMaterialID, rm.Quantity, m.MaterialName, rm.Unit 
                ...
                result.append({
                    ...
                    "Ingredients": [{"RecipeIngredientID": row.RecipeIngredientID, "IngredientName": row.IngredientName, "Amount": row.Amount, "Unit": row.Unit} for row in ingredients],
                    "Materials": [{"RecipeMaterialID": row.RecipeMaterialID, "MaterialName": row.MaterialName, "Quantity": row.Quantity, "Unit": row.Unit} for row in materials]
                })
        return result
    finally:
...
# --- CREATE RECIPE (Management roles only) ---
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            for ing in recipe.Ingredients:
                await cursor.execute("INSERT INTO RecipeIngredients (RecipeID, IngredientID, Amount, Unit) VALUES (?, ?, ?, ?)", new_id, ing.IngredientID, ing.Amount, ing.Unit)

            for mat in recipe.Materials:
                await cursor.execute("INSERT INTO RecipeMaterials (RecipeID, MaterialID, Quantity, Unit) VALUES (?, ?, ?, ?)", new_id, mat.MaterialID, mat.Quantity, mat.Unit)
...
# --- UPDATE RECIPE (Management roles only) ---
...
async def update_recipe(recipe_id: int, recipe: RecipeUpdate, token: str = Depends(oauth2_scheme)):
    ...
    try:
        async with conn.cursor() as cursor:
            ...
            # Simple approach: delete and re-insert components
            await cursor.execute("DELETE FROM RecipeIngredients WHERE RecipeID = ?", (recipe_id,))
            for ing in recipe.Ingredients:
                await cursor.execute("INSERT INTO RecipeIngredients (RecipeID, IngredientID, Amount, Unit) VALUES (?, ?, ?, ?)", recipe_id, ing.IngredientID, ing.Amount, ing.Unit)

            await cursor.execute("DELETE FROM RecipeMaterials WHERE RecipeID = ?", (recipe_id,))
            for mat in recipe.Materials:
                await cursor.execute("INSERT INTO RecipeMaterials (RecipeID, MaterialID, Quantity, Unit) VALUES (?, ?, ?, ?)", recipe_id, mat.MaterialID, mat.Quantity, mat.Unit)
...
# --- Delete recipe (Management roles only) ---
